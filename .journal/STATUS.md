...
Parser robustness: cyclic-linkbase SIGSEGV, percent-encoded paths, primary-statement
detection, XML-comment linkbase aborts and ESEF package resolution all fixed;
`base_sets` dedup and location-key normalisation still open.
...
- [x] Bundled-mirror fallback for un-fetchable W3C schemas — www.w3.org 403s every runtime fetch
- [x] Comment-safe linkbase iteration — one XML comment aborted a whole linkbase (BIC calc 177→264)
- [x] Recover ESEF packages renamed off the mandated LEI name — JM calc_df 0→144, labels 0/20→17/20
- [x] Resolve http refs against the package's own mirror; bounded HTTP timeout (was unbounded)
- [x] Triage missing-vs-broken linkbase in the log — a missing `_ref` is harmless, a missing `_cal` is not
...
- [ ] Rathag-class local miss (`Jahresfinanzbericht 2020/rathag.com/...xsd`) — still undiagnosed
...
- A broad `except` that logs and carries on has now hidden data loss three separate times (percent-encoded paths, the XML-comment linkbase abort, the `base_sets` duplication). The `label` column above is very likely the same shape. Treat "parsed successfully" as unproven whenever a linkbase failed to load.
...
- The running `ureg_3020` re-parse does NOT pick up code fixes: its workers import the modules at launch. Everything since the restart (comment fix, JM-class recovery, package mirror) needs another restart to land. Only cache seeding applies live.
